...
export default function CalendarioPage() {
  ...
  const getDayColor = (day: number) => {
    ...
    if (!dayData) return '';
    if (dayData.block?.blocked) return 'bg-gray-300 text-gray-700';
    if (dayData.reservations.some((r) => r.status === 'CONFIRMED')) return 'bg-red-200 text-red-900';
    if (dayData.reservations.some((r) => r.status === 'PRE_RESERVED' || r.status === 'REQUESTED')) return 'bg-yellow-200 text-yellow-900';
    ...
    <div className="space-y-6">
      ...
      <div className="admin-card">
        <div className="mb-4 flex items-center justify-between">
          <button onClick={() => setCurrentDate(new Date(year, month - 1))} className="rounded p-2 hover:bg-gray-100">
        ...
        <div className="mb-4 flex flex-wrap gap-3 text-xs">
          ...
          <span className="flex items-center gap-1"><span className="h-3 w-3 rounded bg-yellow-200 border border-yellow-300"></span> Pré-reserva</span>
          ...
          <span className="flex items-center gap-1"><span className="h-3 w-3 rounded bg-gray-300 border border-gray-400"></span> Bloqueada</span>
        </div>
          ...
          {Array.from({ length: daysInMonth }).map((_, i) => {
            ...
            return (
              <button
                ...
                className={`relative rounded-lg p-2 text-sm transition-all hover:ring-2 hover:ring-sauna-400 ${colorClass} ${selectedDate === dateStr ? 'ring-2 ring-sauna-600' : ''}`}
              >
                {day}
                {dayData?.reservations?.length ? (
                  <span className="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-sauna-600 text-[10px] text-white">
                    {dayData.reservations.length}
                  </span>
                ) : null}
              </button>
              ...
              {selectedData.reservations.map((r) => (
                <div key={r.id} className="flex items-center justify-between rounded-lg border p-3">
                  <div>
                    ...
                    <p className="text-xs text-gray-500">{r.package?.name || 'Sauna'}</p>
                  </div>
